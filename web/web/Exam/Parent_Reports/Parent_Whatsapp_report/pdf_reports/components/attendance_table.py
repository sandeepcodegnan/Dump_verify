...
def create_attendance_table(student_data, period_id):
    """Create attendance table - week-wise for monthly, single for weekly"""
    
    styles = get_styles()
    story = []
    if "attendance" not in student_data or not student_data['attendance']:
        return story
    ...
    if delta_days > 7:  # Monthly - create week-wise tables
        story.append(Paragraph("Attendance Reports (Week-wise)", styles['SectionTitle']))
        
        # Group dates by weeks
        weekly_groups = group_attendance_by_weeks(att, start_dt, end_dt)
        ...
        for week_num, week_dates in weekly_groups.items():
            ...
            week_table = create_week_attendance_table(week_dates, att, styles)
            story.append(week_table)
            story.append(Spacer(1, 0.2*inch))
        
        # Add overall percentage for monthly
        overall_pct = att.get('overallPercentage', 0)
        overall_data = [[Paragraph('Overall Attendance:', styles['InfoLabel']), Paragraph(f'{overall_pct}%', styles['InfoValue'])]]
        overall_tbl = Table(overall_data, colWidths=[2*inch, 1*inch])
        ...
        legend_data = [[
            ...
            Paragraph('– = No class/Month end', styles['TableBodyLeft'])
        ]]
        ...
        story.append(Paragraph("Attendance Reports", styles['SectionTitle']))
        ...
        weekly_table = create_single_attendance_table(dates, att, styles)
        ...
        legend_data = [[
            ...
            Paragraph('✔ = Present', styles['TableBodyLeft']),
            Paragraph('✘ = Absent', styles['TableBodyLeft']),
            Paragraph('– = No class', styles['TableBodyLeft'])
        ]]
...
def group_attendance_by_weeks(att, start_dt, end_dt):
    """Group attendance dates by fixed Monday-Saturday weeks"""
    weekly_groups = {}
...
def create_week_attendance_table(week_dates, att, styles):
    """Create attendance table for a specific week with fixed Monday-Saturday structure"""
    ...
    header = ["Subjects"] + header_dates + ["Week %"]
    header_row = [Paragraph(col, styles['TableHeader']) for col in header]
    table_data = [header_row]
    
    # Process each subject
    for subj_name, subj_data in att.items():
        if isinstance(subj_data, dict) and subj_name not in ['overallPercentage']:
            row = [Paragraph(subj_name, styles['TableBodyLeft'])]
...
def create_single_attendance_table(dates, att, styles):
    """Create single attendance table for weekly reports"""
    header = ["Subjects"] + dates + ["Overall %"]
    header_row = [Paragraph(col, styles['TableHeader']) for col in header]
    attendance_data = [header_row]
    
    total_pct = 0
    subject_items = [(subj, data) for subj, data in att.items() if isinstance(data, dict) and subj != 'overallPercentage']
    today_str = datetime.today().strftime("%Y-%m-%d")
    
    for subj_name, data in subject_items:
        pct = data.get("percentage", 0) if isinstance(data, dict) else 0
        total_pct += pct
        
        row = [Paragraph(str(subj_name), styles['TableBodyLeft'])]
        for d in dates:
