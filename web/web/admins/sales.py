...
class Sales(Resource):
    def __init__(self):
        ...
        self.collection = get_collection('sales')
        ...
        <body>
            <div class="container">
                <div class="content">
                    ...
                    <p>Hello, {name},</p>
                    <p>Congratulations on joining our team as a Sales Executive!</p>
    ...
    def post(self):
        ...
        if self.mentor_collection.find_one({"email":email}):
            return {"message": " This mail Already Existed in Sales", "status": "error"},404
        ...
        result = self.collection.insert_one(Sales)
        Sales['_id'] = str(result.inserted_id)

        self.send_email(name, email,password)
        return {"message": "Sales signup successful"}, 201
    ...
    def put(self):
        ...
        Sales = self.collection.find_one({"id": id})
        if not Sales:
            return {"error": "Sales with the specified email not found"}, 404
        ...
        updated_Sales = self.collection.find_one({"id": id})
        updated_Sales["_id"] = str(updated_Sales["_id"])

        return {"message": "Sales updated successfully", "Sales": updated_Sales}, 200
    ...
    def delete(self):
        ...
        if result.deleted_count == 0:
            return {"error": "Sales with the specified Id not found"}, 404

        return {"message": "Sales deleted successfully"}, 200

    @admin_required
    def get(self):
        Sales = list(self.collection.find({},{"password":0}))
        for Sale in Sales:
