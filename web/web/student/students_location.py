...
class StudentsByLocation(Resource):
    ...
    def get(self):
        ...
        location  = request.args.get('location')
        try:
            student_document = list(self.student_collection.find({"location":location},{"_id":1,"id":1,"studentId":1, "name":1,"BatchNo":1,"email":1, "studentPhNumber":1,"parentNumber":1,"collegeName":1,"qualification":1,"department":1,"highestGraduationpercentage":1,"yearOfPassing":1,"studentSkills":1,"ArrearsCount":1,"location":1,"placed":1}))
            for data in student_document:
                data['_id'] = str(data['_id'])   
    ...
    #students placed Status
    @manager_required
    def put(self):
        placement = request.json.get('placement')
        studentId = request.json.get('studentId')
        
        if placement is None or studentId is None:
            return {"message": "Missing required fields: placement and student_id"}, 400
        
        result = self.student_collection.update_one({"studentId": studentId},{"$set": {"placed": bool(placement)}})
        
        if result.matched_count == 0:
            return {"message": "Student not found"}, 404
        
        return {"message": "Placement status updated successfully"}, 200